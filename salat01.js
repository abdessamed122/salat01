...
let cities =[
  ...
  for( let city of cities){
    const content =`
    <option>${city.ar_name}</option>
    `

document.getElementById("wilaya1").innerHTML+=content
}

document.getElementById("wilaya1").addEventListener("change",function(){
...
function getprayer(cityname){
    ...
    .then(function (response) {
        ...
        let dohr = response.data.data.timings.Dhuhr
...
const weekdayy =response.data.data.date.hijri.weekday.ar
const date = weekdayy+" "+readableday
document.getElementById("dayy").innerHTML = date
       ...
       console.log(readableday+" "+weekdayy)
